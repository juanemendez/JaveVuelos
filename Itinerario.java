...
public class Itinerario {
	...
	public Trayecto buscarTrayecto(int id){
		for(int i=0;i<this.listaTrayectos.size();i++){
			if(this.listaTrayectos.get(i).getId()==id){
				return this.listaTrayectos.get(i);
			}
		}
		return null;
	}
	...
	public Pasajero buscarPasajero(String id){
		for(int i=0;i<this.listaPasajeros.size();i++){
			if(this.listaPasajeros.get(i).getIdentificacion()==id){
				return this.listaPasajeros.get(i);
			}
		}
		return null;
	}
	...
	public void marcarSillaComoComprada(int idTrayecto,String idSilla,String identificacion){
		Trayecto trayecto = this.buscarTrayecto(idTrayecto);
		Pasajero pasajero = this.buscarPasajero(identificacion);
