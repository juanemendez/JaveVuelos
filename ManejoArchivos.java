...
public class ManejoArchivos {
	...
	public static void ingresarCiudades(SistemaVuelos sistema){
		...
		System.out.println("Ingrese el nombre del archivo texto");
		texto=input.nextLine();

		File inFile=new File(texto);
		...
		try{
			...
			while(!linea.equals("#FIN")){
				...
				while(!linea.equals("0")){
					...
					sistema.agregarVueloPlaneado(codigoAerolinea,codigoVueloPlaneado,numeroVuelo,diaSemana,horaSalida,horaLlegada,codigoCiudadOrigen,codigoCiudadDestino);
					linea = input.nextLine();
					linea = linea.trim();
				}
				linea = input.nextLine();
	...
	public static void generarReporte(SistemaVuelos sistema){
		...
		try{
			...
			if(b){
				System.out.println("Se cre� el archivo de manera exitosa");
			}
			else{
